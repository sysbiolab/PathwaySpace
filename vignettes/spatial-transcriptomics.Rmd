...
`r fa("exclamation-triangle", fill = "orange")` **Note:** If you are new to *Seurat*'s spatial workflows, we recommend reviewing the [spatial analysis tutorials](https://satijalab.org/seurat/articles/get_started_v5_new#spatial-analysis){target="_blank" rel="noopener"} before continuing.
...
The `stxBrain` dataset is normalized as suggested in *Seurat*'s [spatial_vignette](https://satijalab.org/seurat/articles/spatial_vignette.html){target="_blank" rel="noopener"}, either using the `SCTransform()` and `NormalizeData()` functions.
...
```{r Create a GraphSpace, eval=FALSE, message=FALSE}
...
# gg <- plotGraphSpace(pspace_obj, add.image = TRUE,
#   xlab = "Spot coordinates 1", ylab = "Spot coordinates 2")
# ggsave(filename = "./figs_spatl/fig1.png", height=4, width=5,
#   units="in", device="png", dpi=300, plot=gg)
```

```{r fig1, echo=FALSE, out.width = '90%', purl=FALSE}
...
We then perform the signal projection, setting `decay = 0.5`. The decay parameter controls how the signal attenuates as a function of distance in pathway space. With `decay = 0.5`, the signal decreases to half of its initial value at a distance equal to `pdist` (for additional configuration details, see the [*modeling signal decay*](modeling-signal-decay.html) tutorial).
