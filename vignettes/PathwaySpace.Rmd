...
By default, the signal projection uses a Weibull decay function, which is passed to the `circularProjection()` function via the vertex attribute `decayFunction`. This attribute can be modified by the user using the `gs_vertex_attr()` accessor. For example:

```{r Circular projection - 3, eval=FALSE, message=FALSE, out.width="70%"}
# Modify the vertex `decayFunction` attribute, adjusting Weibull's 'shape'
gs_vertex_attr(p_space1, "decayFunction") <- signalDecay(shape = 2)
...
gs_vertex_attr(p_space2, "decayFunction") <- signalDecay(shape = 2)
```
