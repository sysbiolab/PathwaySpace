...
In this tutorial we use the low-level *ggplot2* interface for fine-grained control; the following tutorials demonstrate the higher-level `plotPathwaySpace()` wrapper for convenience.
...
# ggsave(filename = "./figs_spatl/fig1.png", height=4, width=7,
...
Next, we specify the signal to be projected; for this demonstration, we will use expression data from the **Camk2n1** gene. The `activeFeature()` accessor function is then used to assign the gene expression values to graph vertices.
