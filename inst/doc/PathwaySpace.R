...
#  library(igraph)
#  
#  #--- Load other required packages that may need instalation
...
top10hubs <- degree(PCv12_pruned_igraph)
top10hubs <- names(sort(top10hubs, decreasing = TRUE)[1:10])
head(top10hubs)
# [1] "GNB1"   "TRIM28" "RPS27A" "CTNNB1" "TP53"   "ACTB"

## ----PathwaySpace decoration - 2, eval=TRUE, message=FALSE, out.width="50%"----
# Mark top-connected nodes in the igraph object
V(PCv12_pruned_igraph)$label <- NA
idx <- match(top10hubs, V(PCv12_pruned_igraph)$name)
V(PCv12_pruned_igraph)$label[idx] <- top10hubs

# Plot the igraph object labeled with top-connected nodes
plot.igraph(PCv12_pruned_igraph)
title(main = "Pruned igraph with\nn=12990 vertices from
  Pathway Commons V12", cex.main=1)

## ----PathwaySpace decoration - 3, eval=FALSE, message=FALSE-------------------
#  # Load Hallmark gene sets
#  gsco <- msigdbr(species = "Homo sapiens", category=c("H"))
#  
#  # Make a list with Hallmark genes
#  gsco <- split(gsco$human_gene_symbol, gsco$gs_name)
#  
#  # There are 50 gene sets in the 'gsco' list
#  length(gsco)
#  # [1] 50
#  
#  # We will use the 'HALLMARK_P53_PATHWAY' (n=215 genes) for demonstration
#  length(gsco$HALLMARK_P53_PATHWAY)
#  # [1] 215
...
#  gsco <- lapply(gsco, intersect, y = names(pspace_PCv12) )
#  
...
#  vertexSignal(pspace_PCv12)[ gsco$HALLMARK_P53_PATHWAY ] <- 1
...
#    marks = top10hubs, mark.size = 2, theme = "th2")
